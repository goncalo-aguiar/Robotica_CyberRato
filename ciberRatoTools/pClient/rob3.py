...
class MyRob(CRobLinkAngs):
    ...
    def wander(self):
        ...
        if somador== 0 and len(self.dic_Intersecoes.values()) >4  and condicao == 'true' :
            ...
            # if len(dic2) == 3:
            #     self.algoritmo("grafo.txt",dic2.get(0),dic2.get(2))
            #     self.algoritmo("grafo.txt",dic2.get(2),dic2.get(1))
            #     self.algoritmo("grafo.txt",dic2.get(1),dic2.get(0))
            ...
            #     distancia_auxiliar = 0
            #     distt = 1000
            #     for j in range(i+1,len(dic2)):
            #         distancia_auxiliar = self.algoritmo1("grafo.txt",dic2.get(lista_path[-1]),dic2.get(j))
                    
            #         if distancia_auxiliar < distt and distancia_auxiliar != 0 and j not in lista_path:
        ...
        if self.atualizar_ficheiro > 150 and self.flag == 'frente':
            ...
            for x in self.coord_beacons.keys():
                string = x.replace("(","")
                string = string.replace(")","")
                string = string.replace(","," ")
                string = string.split()
                print(string[0])
                print(string[1])
                print(self.coord_beacons.get(x))
                matriz_aux[int(string[0])][int(string[1])] = self.coord_beacons.get(x)
            for x in matriz_aux:
                for y in x:
                    f = open("myrob.map", "a")
                    if y=='T' or y =='o' or y=='+':
